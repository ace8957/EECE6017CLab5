...
/**
 * Scrolls '1337' on the HEX displays, does a rotation every
 * 3 seconds.
 */
void scroll1337(void *pdata)
{
	...
	while(1) {
		int i = 0;
		// This loop writes one character of the word, starting at start
		for(i = 0; i < sizeof(word); ++i) {
			IOWR_ALTERA_AVALON_PIO_DATA(displays[start], word[i]);
			// Since start must be in the range 0-3, use modulus arith
			start = (start + 1)%(sizeof(displays)/4);
		}
		// Move the start to one after the last one
		start = (start + 1)%(sizeof(displays)/4);
		// Wait 3 seconds
...
/**
 * Rotates one green LED across the array. It moves
 * every 0.5 seconds.
 */
void rotateGreen(void *pdata)
{
	...
	while(1) {
		...
		disp = disp << 1;
		// If the rotate overflows, set it to 1
		if(disp == 0)
			disp = 1;
